...
class TxnMallTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        rawtx_input_0["vout"] = find_output(self.nodes[0], fund_foo_txid, 87)
        rawtx_input_1 = {}
        rawtx_input_1["txid"] = fund_bar_txid
        rawtx_input_1["vout"] = find_output(self.nodes[0], fund_bar_txid, 2)
        inputs = [rawtx_input_0, rawtx_input_1]
        change_address = self.nodes[0].getnewaddress()
        outputs = {}
        outputs[node1_address] = 80
        outputs[change_address] = 89 - 80 + doublespend_fee
        ...
        txid2 = self.nodes[0].sendfrom("bar", node1_address, 2, 0)
        ...
        # Node0's balance should be starting balance, plus coinbase for another
        # matured block, minus 40, minus 2, and minus transaction fees:
        ...
        # Node0's total balance should be starting balance, plus coinbases for
        # two more matured blocks, minus 80 for the double-spend, plus fees (which are
        # negative):
        expected = starting_balance[0] + block_reward(101) + block_reward(102) - 80 \
        ...
        assert_equal(self.nodes[0].getbalance("foo"), 87)
        assert_equal(self.nodes[0].getbalance("bar"), 2)
        assert_equal(self.nodes[0].getbalance(""), starting_balance[0]
                                                              - 87
                                                              -  2
                                                              - 80
        ...
        assert_equal(self.nodes[1].getbalance("from0"), 80)
