...
static void MutateTxVersion(CEDCMutableTransaction& tx, const string& cmdVal)
{
    int64_t newVersion = atoi64(cmdVal);
    if (newVersion < 1 || newVersion > CEDCTransaction::CURRENT_VERSION)
        throw runtime_error("Invalid TX version requested");

    tx.nVersion = (int) newVersion;
}

static void MutateTxLocktime(CEDCMutableTransaction& tx, const string& cmdVal)
{
...
static void MutateTxAddInput(CEDCMutableTransaction& tx, const string& strInput)
{
...
static void MutateTxAddOutAddr(CEDCMutableTransaction& tx, const string& strInput)
{
    ...
    CEDCTxOut txout(value, scriptPubKey);
    tx.vout.push_back(txout);
}

static void MutateTxAddOutData(CEDCMutableTransaction& tx, const string& strInput)
{
    CAmount value = 0;
    ...
    CEDCTxOut txout(value, CScript() << OP_RETURN << data);
    tx.vout.push_back(txout);
}

static void MutateTxAddOutScript(CEDCMutableTransaction& tx, const string& strInput)
{
    ...
    CScript scriptPubKey = edcParseScript(strScript); // throws on err
...
static void MutateTxDelInput(CEDCMutableTransaction& tx, const string& strInIdx)
{
...
static void MutateTxDelOutput(CEDCMutableTransaction& tx, const string& strOutIdx)
{
    // parse requested deletion index
...
static void MutateTxSign(CEDCMutableTransaction& tx, const string& flagStr)
{
    ...
    vector<CEDCTransaction> txVariants;
...
static void MutateTx(CEDCMutableTransaction& tx, const string& command,
...
static void OutputTxJSON(const CEDCTransaction& tx)
{
...
static void OutputTxHash(const CEDCTransaction& tx)
{
...
static void OutputTxHex(const CEDCTransaction& tx)
{
...
static void OutputTx(const CEDCTransaction& tx)
{
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try 
	{
        ...
        if (!fCreateBlank) 
		{
            ...
            // param: hex-encoded equibit transaction
        ...
        CEDCMutableTransaction tx(txDecodeTmp);
