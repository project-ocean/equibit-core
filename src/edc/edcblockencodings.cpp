...
#define MIN_TRANSACTION_BASE_SIZE (::GetSerializeSize(CEDCTransaction(), SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS))

CEDCBlockHeaderAndShortTxIDs::CEDCBlockHeaderAndShortTxIDs(const CEDCBlock& block) :
...
void CEDCBlockHeaderAndShortTxIDs::FillShortTxIDSelector() const 
{
...
uint64_t CEDCBlockHeaderAndShortTxIDs::GetShortID(const uint256& txhash) const 
{
...
EDCReadStatus EDCPartiallyDownloadedBlock::InitData(const CEDCBlockHeaderAndShortTxIDs& cmpctblock) 
{
...
bool EDCPartiallyDownloadedBlock::IsTxAvailable(size_t index) const 
{
...
EDCReadStatus EDCPartiallyDownloadedBlock::FillBlock(
	CEDCBlock& block, 
	const std::vector<CEDCTransaction>& vtx_missing ) const 
{
    assert(!header.IsNull());
    block = header;
