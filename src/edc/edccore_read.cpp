...
bool DecodeHexTx(CEDCTransaction& tx, const std::string& strHexTx, bool fTryNoWitness )
{
    if (!IsHex(strHexTx))
        return false;
...
bool DecodeHexBlk(CEDCBlock& block, const std::string& strHexBlk)
{
    if (!IsHex(strHexBlk))
        return false;
...
uint256 edcParseHashStr(const std::string& strHex, const std::string& strName)
{
