...
const uint64_t RANDOMIZER_ID_ADDRESS_RELAY = 0x3cac0035b5866b90ULL; // SHA256("main address relay")[0:8]

/** Maximum number of unconnecting headers announcements before DoS score */
const int EDC_MAX_UNCONNECTING_HEADERS = 10;
...
struct QueuedBlock 	
{
    ...
    std::unique_ptr<EDCPartiallyDownloadedBlock> partialBlock;  //!< Optional, used for CMPCTBLOCK downloads
};
...
list<QueuedBlock>::iterator ** pit = NULL) 
{
    ...
    list<QueuedBlock>::iterator it = state->vBlocksInFlight.insert(state->vBlocksInFlight.end(),
            {hash, pindex, pindex != NULL, std::unique_ptr<EDCPartiallyDownloadedBlock>(pit ? new EDCPartiallyDownloadedBlock(&theApp.mempool()) : NULL)});
...
bool AddOrphanTx(const CEDCTransaction& tx, NodeId peer) EXCLUSIVE_LOCKS_REQUIRED(EDC_cs_main)
{
    ...
    unsigned int sz = edcGetTransactionWeight(tx);
    if (sz >= EDC_MAX_STANDARD_TX_WEIGHT)
    {
                                 ...
                                 int64_t nAdjustedTime)
{
    ...
    if((block.nVersion < 2 && nHeight >= consensusParams.BIP34Height) ||
       ...
       (block.nVersion < 4 && nHeight >= consensusParams.BIP65Height))
            return state.Invalid(false, REJECT_OBSOLETE, strprintf("bad-version(0x%08x)", 
				block.nVersion), strprintf("rejected nVersion=0x%08x block", block.nVersion));
		return true;
}
		  ...
		  const CBlockIndex * pindexPrev)
{
	...
	if (VersionBitsState(pindexPrev, consensusParams, Consensus::DEPLOYMENT_SEGWIT, 
		theApp.versionbitscache()) == THRESHOLD_ACTIVE)
	{
        ...
        if (commitpos != -1) 
		{
            ...
            uint256 hashWitness = edcBlockWitnessMerkleRoot(block, &malleated);
    ...
    // failed).
    if (edcGetBlockWeight(block) > EDC_MAX_BLOCK_WEIGHT) 
	{
...
void edcUpdateUncommittedBlockStructures(
				  CEDCBlock & block, 
		  const CBlockIndex * pindexPrev, 
	const Consensus::Params & consensusParams)
{
...
std::vector<unsigned char> edcGenerateCoinbaseCommitment(
			  CEDCBlock & block, 
	  const CBlockIndex * pindexPrev, 
const Consensus::Params & consensusParams)
{
    ...
    if (fHaveWitness && IsWitnessEnabled(pindexPrev, consensusParams)) 
	{
        if (commitpos == -1) 
		{
            uint256 witnessroot = edcBlockWitnessMerkleRoot(block, NULL);
            ...
            const_cast<std::vector<CEDCTxOut>*>(&block.vtx[0].vout)->push_back(out);
            block.vtx[0].UpdateHash();
        }
    }

    edcUpdateUncommittedBlockStructures(block, pindexPrev, consensusParams);
    return commitment;
}
			  ...
			  CEDCConnman & connman)
{
			...
			!theApp.reindex()) // Ignore blocks received while importing
    {
                ...
                if (!MarkBlockAsInFlight(pfrom->GetId(), pindex->GetBlockHash(), chainparams.GetConsensus(), pindex, &queuedBlockIt)) 
				{
                    if (!(*queuedBlockIt)->partialBlock)
                        (*queuedBlockIt)->partialBlock.reset(
							new EDCPartiallyDownloadedBlock(&theApp.mempool()));
                    else 
					{
                        // The block was already in flight using compact blocks from the same peer
                        edcLogPrint("net", "Peer sent us compact block we were already syncing!\n");
                        return true;
                    }
                }

                EDCPartiallyDownloadedBlock& partialBlock = *(*queuedBlockIt)->partialBlock;
                EDCReadStatus status = partialBlock.InitData(cmpctblock);
        ...
        edcCheckBlockIndex(chainparams.GetConsensus());
			...
			!theApp.importing() && !theApp.reindex()) // Ignore blocks received while importing
    {
		...
		{
            edcLogPrint("net", "Peer %d sent us block transactions for block we weren't expecting\n", pfrom->id);
            return true;
        }

        EDCPartiallyDownloadedBlock& partialBlock = *it->second.second->partialBlock;
        CEDCBlock block;
        EDCReadStatus status = partialBlock.FillBlock(block, resp.txn);
        if (status == READ_STATUS_INVALID) 
		{
    ...
    else if (strCommand == NetMsgType::HEADERS && !theApp.importing() && !theApp.reindex() ) // Ignore headers received while importing
    {
			...
			nCount < MAX_BLOCKS_TO_ANNOUNCE) 
		{
            ...
            if (nodestate->nUnconnectingHeaders % EDC_MAX_UNCONNECTING_HEADERS == 0) 
			{
                edcMisbehaving(pfrom->GetId(), 20);
            }
            return true;
		...
		{
            edcLogPrint("net", "peer=%d: resetting nUnconnectingHeaders (%d -> 0)\n", 
				pfrom->id, nodestate->nUnconnectingHeaders);
        }
		...
		theApp.chainActive().Tip()->nChainWork <= pindexLast->nChainWork) 
		{
			...
			else 
			{
                ...
                BOOST_REVERSE_FOREACH(CBlockIndex *pindex, vToFetch) 
				{
                    ...
                    uint32_t nFetchFlags = GetFetchFlags(pfrom, pindex->pprev, 
