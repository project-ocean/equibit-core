...
CEDCPartialMerkleTree::CEDCPartialMerkleTree() : nTransactions(0), fBad(true) 
{}

uint256 CEDCPartialMerkleTree::ExtractMatches(std::vector<uint256> &vMatch, std::vector<unsigned int> &vnIndex) 
{
    vMatch.clear();
    ...
    if (nTransactions > EDC_MAX_BLOCK_BASE_SIZE / 60) // 60 is the lower bound for the size of a serialized CTransaction
        return uint256();
