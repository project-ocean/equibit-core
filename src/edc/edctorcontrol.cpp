...
const std::string EDC_DEFAULT_TOR_CONTROL = "127.0.0.1:9051";
...
public:
	 ...
	 * established. disconnected is a handler that is called when the 
...
void TorControlConnection::readcb(struct bufferevent *bev, void *ctx)
{
		...
		!= NULL)
    {
        ...
        if (ch == ' ') 
		{
			...
			else 
			{
				...
				{
                    edcLogPrint("tor", "tor: Received unexpected sync reply %i\n", self->message.code);
                }
            }
            self->message.Clear();
        }
	...
	{
        edcLogPrintf("tor: Disconnecting because MAX_LINE_LENGTH exceeded\n");
        self->Disconnect();
    }
}
				  ...
				  void * ctx)
{
	...
	{
        edcLogPrint("tor", "tor: Successfully connected!\n");
        self->connected(*self);
    } 
	else if (what & (BEV_EVENT_EOF|BEV_EVENT_ERROR)) 
	{
        if (what & BEV_EVENT_ERROR)
            edcLogPrint("tor", "tor: Error connecting to Tor control socket\n");
        else
            edcLogPrint("tor", "tor: End of stream\n");
	...
	const ConnectionCB & disconnected)
{
	...
	{
        edcLogPrintf("tor: Error parsing socket address %s\n", target);
        return false;
    }
    ...
    if (bufferevent_socket_connect(b_conn, 
		(struct sockaddr*)&connect_to_addr, connect_to_addrlen) < 0) 
	{
        edcLogPrintf("tor: Error connecting to address %s\n", target);
        return false;
    }
    return true;
}
...
/****** Equibit specific TorController implementation ********/
    ...
    reconnect_timeout(RECONNECT_TIMEOUT_START)
{
    ...
    if (!reconnect_ev)
        edcLogPrintf("tor: Failed to create event for reconnection: out of memory?\n");

    // Start connection attempts immediately
    if (!conn.Connect(target, boost::bind(&TorController::connected_cb, this, _1),
         boost::bind(&TorController::disconnected_cb, this, _1) )) 
	{
        	edcLogPrintf("tor: Initiating connection to Tor control port %s failed\n", target);
    }
    ...
    if (pkf.first) 
	{
        edcLogPrint("tor", "tor: Reading cached private key from %s\n", GetPrivateKeyFile());
...
void TorController::add_onion_cb(TorControlConnection& conn, const TorControlReply& reply)
{
    if (reply.code == 250) 
	{
        edcLogPrint("tor", "tor: ADD_ONION successful\n");
		...
		{
            edcLogPrint("tor", "tor: Cached service private key to %s\n", GetPrivateKeyFile());
        } 	
		else 
		{
            edcLogPrintf("tor: Error writing service private key to %s\n", GetPrivateKeyFile());
        }
        edcAddLocal(service, LOCAL_MANUAL);
        // ... onion requested - keep connection open
    } 
	else if (reply.code == 510) 
	{ 
		// 510 Unrecognized command
        edcLogPrintf("tor: Add onion failed with unrecognized command (You probably need to upgrade Tor)\n");
    } 
	else 
	{
        edcLogPrintf("tor: Add onion failed; error code %d\n", reply.code);
    }
}

void TorController::auth_cb(
	 TorControlConnection & conn, 
	const TorControlReply & reply)
{
	...
	{
        edcLogPrintf("tor: Authentication failed\n");
    }
}
...
void TorController::authchallenge_cb(
	 TorControlConnection & conn, 
	const TorControlReply & reply)
{
    if (reply.code == 250) 
	{
        edcLogPrint("tor", "tor: SAFECOOKIE authentication challenge successful\n");
        ...
        if (l.first == "AUTHCHALLENGE") 
		{
            ...
            edcLogPrint("tor", "tor: AUTHCHALLENGE ServerHash %s ServerNonce %s\n", HexStr(serverHash), HexStr(serverNonce));

            if (serverNonce.size() != 32) 
			{
                edcLogPrintf("tor: ServerNonce is not 32 bytes, as required by spec\n");
                return;
            }
			...
			{
                edcLogPrintf("tor: ServerHash %s does not match expected ServerHash %s\n", HexStr(serverHash), HexStr(computedServerHash));
                return;
            }
		...
		{
            edcLogPrintf("tor: Invalid reply to AUTHCHALLENGE\n");
        }
    } 
	else 
	{
        edcLogPrintf("tor: SAFECOOKIE authentication challenge failed\n");
    }
}
    ...
    if (reply.code == 250) 
	{
        ...
        BOOST_FOREACH(const std::string &s, reply.lines) 
		{
			...
			else if (l.first == "VERSION") 
			{
				...
				{
                    edcLogPrint("tor", "tor: Connected to Tor version %s\n", i->second);
                }
            }
        }
		...
		{
            edcLogPrint("tor", "tor: Supported authentication method: %s\n", s);
        }
		...
		else if (methods.count("SAFECOOKIE")) 
		{
            ...
            edcLogPrint("tor", "tor: Using SAFECOOKIE authentication, reading cookie authentication from %s\n", cookiefile);

            std::pair<bool,std::string> status_cookie = ReadBinaryFile(cookiefile, TOR_COOKIE_SIZE);

            if (status_cookie.first && status_cookie.second.size() == 
				TOR_COOKIE_SIZE) 
			{
			...
			else 
			{
				...
				{
                    edcLogPrintf("tor: Authentication cookie %s is not exactly %i bytes, as is required by the spec\n", cookiefile, TOR_COOKIE_SIZE);
                } 
				else 
				{
                    edcLogPrintf("tor: Authentication cookie %s could not be opened (check permissions)\n", cookiefile);
                }
            }
        } 
		...
		{
            edcLogPrintf("tor: No supported authentication method\n");
        }
    } 
	else 
	{
        edcLogPrintf("tor: Requesting protocol info failed\n");
    }
}
...
void TorController::connected_cb(TorControlConnection& conn)
{
    ...
    if (!conn.Command("PROTOCOLINFO 1", boost::bind(&TorController::protocolinfo_cb, this, _1, _2)))
        edcLogPrintf("tor: Error sending initial protocolinfo command\n");
}
...
void TorController::disconnected_cb(TorControlConnection& conn)
{
    ...
    edcLogPrint("tor", "tor: Not connected to Tor control port %s, trying to reconnect\n", target);
...
void TorController::Reconnect()
{
    ...
    if (!conn.Connect(target, boost::bind(&TorController::connected_cb, this, 
		_1), boost::bind(&TorController::disconnected_cb, this, _1) )) 
	{
        edcLogPrintf("tor: Re-initiating connection to Tor control port %s failed\n", target);
    }
}
...
{
	EDCparams & params = EDCparams::singleton();
    TorController ctrl(base, params.torcontrol );
...
void edcStartTorControl(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (!base) 
	{
        edcLogPrintf("tor: Unable to create event_base\n");
        return;
    }
...
void edcInterruptTorControl()
{
    if (base) 
	{
        edcLogPrintf("tor: Thread interrupt\n");
