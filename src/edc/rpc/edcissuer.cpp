...
UniValue authorizeEquibit(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 5)
        throw std::runtime_error(
            R"__(authorizeequibit "issuer" amount wot_min_level ("comment") (subtract_fee_from_amount)
 ...
 3. wot_min_level             (numeric, required) The minimum WoT level used when TXN is moved.
 ...
 transactionid                (string) The id of the generated transaction.)__"
    ...
    if (!pwalletMain->CreateAuthorizingTransaction(issuer, WoTlvl, vecSend, wtxNew, reservekey, nFeeRequired, nChangePosRet, strError))
    {
...
UniValue blankEquibit(const JSONRPCRequest& request)
{
 ...
 transactionid                (string) The id of the generated transaction.)__"
    ...
    if (!pwalletMain->CreateBlankingTransaction(issuer, vecSend, wtxNew, reservekey, feeFromBlank, nFeeRequired, nChangePosRet, strError))
    {
        if (amount > pwalletMain->GetBalance())
            strError = strprintf("Error: This transaction requires a transaction fee of at least %s because of its amount, complexity, or use of recently received funds!", FormatMoney(nFeeRequired));
...
const CRPCCommand commands[] =
{
    ...
    { "equibit", "authorizeequibit", &authorizeEquibit,  true, { "issuer", "amount", "wot_min_level", "comment", "subtract_fee_from_amount" } },
    { "equibit", "blankequibit",     &blankEquibit,      true, { "issuer", "amount", "comment", "subtract_fee_from_amount", "fee_from_blank" } },
};
