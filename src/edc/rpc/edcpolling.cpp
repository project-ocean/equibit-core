...
//
bool validAnswers(const std::string& answers, std::vector<std::string>& ansVec)
{
    if (answers.size() == 0 ||
        answers[0] == ',' ||
...
UniValue pollresults(const JSONRPCRequest& request)
{
    ...
    if (type == "summary")       result->summary(ans);
    else if (type == "response") result->response(ans);
    else if (type == "all")      result->all(ans);
    else
...
void PollResult::addVote(const std::string& ans, const CKeyID& id, Type ty)
{
