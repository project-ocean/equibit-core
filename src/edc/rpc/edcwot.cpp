...
void addressToPubKey(const std::string& addr, CPubKey& pubkey, EDCapp&)
{
    ...
    if (!pwalletMain->GetPubKey(pkeyID, pubkey))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Address of public key does not refer to key");
}
...
void insertStr(std::vector<unsigned char>::iterator& it, const std::string& str)
{
...
UniValue getwotcertificate(const JSONRPCRequest& request)
{
    ...
    WoTCertificate cert(
        pkAddrs, saddr,
        oname, ogaddr, ophone, oemail, ohttp,
        sname, sgaddr, sphone, semail, shttp,
        expire);
