        ...
        while (pc < pend)
        {
            ...
            if (fExec && 0 <= opcode && opcode <= OP_PUSHDATA4) 
			{
                ...
                case OP_CHECKSEQUENCEVERIFY:
                {
                    ...
                    if ((nSequence & CEDCTxIn::SEQUENCE_LOCKTIME_DISABLE_FLAG) != 0)
                        break;
...
/**
 * Wrapper that serializes like CEDCTransaction, but with the modifications
 *  required for the signature hash done in-place
 */
class CEDCTransactionSignatureSerializer 
{
private:
    const CEDCTransaction& txTo;  //!< reference to the spending transaction (the one being serialized)
...
public:
    CEDCTransactionSignatureSerializer(const CEDCTransaction &txToIn, const CScript &scriptCodeIn, unsigned int nInIn, int nHashTypeIn) :
    ...
    void SerializeOutput(S &s, unsigned int nOutput, int nType, int nVersion) const 
	{
        if (fHashSingle && nOutput != nIn)
            ...
            ::Serialize(s, CEDCTxOut(), nType, nVersion);
        else
 ...
 const EDCPrecomputedTransactionData * cache)
{
    ...
    CEDCTransactionSignatureSerializer txTmp(txTo, scriptCode, nIn, nHashType);
...
bool EDCTransactionSignatureChecker::CheckLockTime(const CScriptNum& nLockTime) const
{
    ...
    if (CEDCTxIn::SEQUENCE_FINAL == txTo->vin[nIn].nSequence)
        return false;

    return true;
}

bool EDCTransactionSignatureChecker::CheckSequence(const CScriptNum& nSequence) const
{
    ...
    if (txToSequence & CEDCTxIn::SEQUENCE_LOCKTIME_DISABLE_FLAG)
        return false;

    // Mask off any bits that do not have consensus-enforced meaning
    // before doing the integer comparisons
    const uint32_t nLockTimeMask = CEDCTxIn::SEQUENCE_LOCKTIME_TYPE_FLAG | CEDCTxIn::SEQUENCE_LOCKTIME_MASK;
    const int64_t txToSequenceMasked = txToSequence & nLockTimeMask;
    const CScriptNum nSequenceMasked = nSequence & nLockTimeMask;

    // There are two kinds of nSequence: lock-by-blockheight
    // and lock-by-blocktime, distinguished by whether
    // nSequenceMasked < CEDCTxIn::SEQUENCE_LOCKTIME_TYPE_FLAG.
    //
    // We want to compare apples to apples, so fail the script
    // unless the type of nSequenceMasked being tested is the same as
    // the nSequenceMasked in the transaction.
    if (!(
        (txToSequenceMasked <  CEDCTxIn::SEQUENCE_LOCKTIME_TYPE_FLAG && 
			nSequenceMasked <  CEDCTxIn::SEQUENCE_LOCKTIME_TYPE_FLAG) ||
        (txToSequenceMasked >= CEDCTxIn::SEQUENCE_LOCKTIME_TYPE_FLAG && 
			nSequenceMasked >= CEDCTxIn::SEQUENCE_LOCKTIME_TYPE_FLAG)
    )) 
...
)
{
    ...
    if (witversion == 0) 
	{
        if (program.size() == 32) 
		{
            ...
            CSHA256().Write(&scriptPubKey[0], scriptPubKey.size()).Finalize(
				hashScriptPubKey.begin());
		...
		else if (program.size() == 20) 
		{
			...
			{
                return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH); // 2 items in witnes
            }
			   ...
			   ScriptError * serror)
{
    ...
    if ((flags & SCRIPT_VERIFY_P2SH) && scriptPubKey.IsPayToScriptHash())
    {
        ...
        // an empty stack and the edcEvalScript above would return false.
        assert(!stack.empty());
        ...
        if (flags & SCRIPT_VERIFY_WITNESS) 
		{
			...
			{
                hadWitness = true;
                if (scriptSig != CScript() << std::vector<unsigned char>(pubKey2.begin(), 
				pubKey2.end())) 
				{
