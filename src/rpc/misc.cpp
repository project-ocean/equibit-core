...
#else // BUILD_OCN
			"\nReturn information about the given ocean address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The ocean address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\",        (string) The ocean address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
#endif // END_BUILD
...
#else // BUILD_OCN
            "to the wallet before upgrading to v0.17. To use the deprecated functionality, start oceand with -deprecatedrpc=createmultisig\n"
#endif // END_BUILD
            "\nArguments:\n"
...
#else // BUILD_OCN
            " To use the deprecated functionality, start oceand with -deprecatedrpc=createmultisig", keys[i].get_str()));
#endif // END_BUILD
        }
    }
...
#else // BUILD_OCN
            "1. \"address\"         (string, required) The ocean address to use for the signature.\n"
#endif // END_BUILD
...
#else // BUILD_OCN
            "ocean-cli and the GUI. There is no server-side difference."
#endif // END_BUILD
        );
...
#else // BUILD_OCN
        "\nocean-cli has the option -getinfo to collect and format these in the old format."
#endif // END_BUILD
    );
}
