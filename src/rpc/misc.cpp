...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"     (string, required) The equibit address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\",        (string) The equibit address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
#endif // END_BUILD
...
#else // BUILD_EQB
            "to the wallet before upgrading to v0.17. To use the deprecated functionality, start equibitd with -deprecatedrpc=createmultisig\n"
#endif // END_BUILD
            "\nArguments:\n"
...
#else // BUILD_EQB
            " To use the deprecated functionality, start equibitd with -deprecatedrpc=createmultisig", keys[i].get_str()));
#endif // END_BUILD
        }
    }
...
#else // BUILD_EQB
            "1. \"address\"         (string, required) The equibit address to use for the signature.\n"
#endif // END_BUILD
...
#else // BUILD_EQB
            "equibit-cli and the GUI. There is no server-side difference."
#endif // END_BUILD
        );
...
#else // BUILD_EQB
        "\nequibit-cli has the option -getinfo to collect and format these in the old format."
#endif // END_BUILD
    );
}
