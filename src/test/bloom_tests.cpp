...
BOOST_AUTO_TEST_CASE(bloom_match)
{
...
#else // BUILD_EQB
    //! EQB_TODO: Uncomment and enable fixtures and tests
    /*
    ...
    // !EQB_TODO: Fix Test-> CTransaction tx(deserialize, stream);
    ...
    // !EQB_TODO: Fix Test-> BOOST_CHECK_MESSAGE(filter.IsRelevantAndUpdate(tx), "Simple Bloom filter didn't match tx hash");
    */
#endif // END_BUILD

    // !EQB_TODO: Uncomment -> filter = CBloomFilter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
#ifdef BUILD_BTC
...
#else // BUILD_EQB
    // byte-reversed tx hash
    // !EQB_TODO: Uncomment -> filter.insert(ParseHex("7c037e163f8dfee4632a8cf6c87187d3cb61224e6dae8f4b0ed0fae3a3800857")); // id of first tnx
    // !EQB_TODO: Fix Test-> BOOST_CHECK_MESSAGE(filter.IsRelevantAndUpdate(tx), "Simple Bloom filter didn't match manually serialized tx hash");
#endif // END_BUILD
    ...
    // !EQB_TODO: Fix Test-> (filter.IsRelevantAndUpdate(tx), "Simple Bloom filter didn't match output address");

    // !EQB_TODO: Fix Test-> BOOST_CHECK_MESSAGE(filter.IsRelevantAndUpdate(spendingTx), "Simple Bloom filter didn't add output");
    */
#endif // END_BUILD
#ifdef BUILD_BTC
    filter = CBloomFilter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
 ...
 /*
    ...
    // !EQB_TODO: Fix Test-> BOOST_CHECK_MESSAGE(filter.IsRelevantAndUpdate(tx), "Simple Bloom filter didn't match output address");

    filter = CBloomFilter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
    filter.insert(COutPoint(uint256S("0x2e203ff0c411db27b2f77da9287a1c77606a35d3143297de361529537484cdef"), 0));
    // !EQB_TODO: Fix Test-> BOOST_CHECK_MESSAGE(filter.IsRelevantAndUpdate(tx), "Simple Bloom filter didn't match COutPoint");
    ...
    // !EQB_TODO: Fix Test-> BOOST_CHECK_MESSAGE(filter.IsRelevantAndUpdate(tx), "Simple Bloom filter didn't match manually serialized COutPoint");

    filter = CBloomFilter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
    filter.insert(uint256S("00000007226335f9d30f4197bf7034db711b1c2a16c31055939eef62e498ad2a"));
    // !EQB_TODO: Fix Test-> BOOST_CHECK_MESSAGE(!filter.IsRelevantAndUpdate(tx), "Simple Bloom filter matched random tx hash");

    filter = CBloomFilter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
    filter.insert(ParseHex("000000c6a74b7a8fc4c13c51dc2cb048d012f5cc"));
    // !EQB_TODO: Fix Test-> BOOST_CHECK_MESSAGE(!filter.IsRelevantAndUpdate(tx), "Simple Bloom filter matched random address");

    filter = CBloomFilter(10, 0.000001, 0, BLOOM_UPDATE_ALL);
    filter.insert(COutPoint(uint256S("0x2e203ff0c411db27b2f77da9287a1c77606a35d3143297de361529537484cdef"), 1));
    // !EQB_TODO: Fix Test-> BOOST_CHECK_MESSAGE(!filter.IsRelevantAndUpdate(tx), "Simple Bloom filter matched COutPoint for an output we didn't care about");
