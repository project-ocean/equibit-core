...
#ifndef BUILD_BTC
void TestHMACSHA3(const std::string &hexkey, const std::string &hexin, const std::string &hexout) {
    std::vector<unsigned char> key = ParseHex(hexkey);
    TestVector(CHMAC_SHA3(key.data(), key.size()), ParseHex(hexin), ParseHex(hexout));
}
#endif
...
#ifndef BUILD_BTC
BOOST_AUTO_TEST_CASE(hmac_sha3_testvectors) {
    // test cases generated by eqb/scripts/hmac-sha3.py
