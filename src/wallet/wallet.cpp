...
std::string CWalletTx::toJSON(const char* margin) const
{
    ...
    ans << innerMargin;
    ans << "\"merkleTx\":\n";
    ans << CMerkleTx::toJSON(innerMargin.c_str());
...
/*
vCoins              // OUT: Coins available for input to TXN
issuer              // IN:  authorizing issuer
wotlvl              // IN:  WoT level
fOnlyConfirmed      // IN:  If true, then only trusted/confirmed coins ret'd
coinControl         // IN:  Controls coins selected
fIncludeZeroValue   // IN:  If true, then zero value coins included
*/
{
    CKeyID issuerID;
...
std::string CMerkleTx::toJSON(const char* margin) const
{
    std::stringstream ans;
    ...
    ans << CTransaction().toJSON(innerMargin.c_str());
...
bool CWallet::CreateAuthorizingTransaction(
...
//          T                 T            V-f        f        V-f     f
//
bool CWallet::CreateBlankingTransaction(
    const CIssuer& issuer,
    const std::vector<CRecipient>& vecSend,
    CWalletTx& wtxNew,
    CReserveKey& reservekey,
    ...
    std::string& strFailReason)
{
...

    {
        ...
        {
            std::vector<COutput> vAvailableCoins;
            std::vector<COutput> vAvailableBlankCoins;
            ...
            while (true)
            {
                ...
                if (nSubtractFeeFromAmount == 0 && !feeFromBlank) nValueToSelect += nFeeRet;
                ...
                BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setBlankCoins)
                {
                    CAmount nCredit = pcoin.first->tx->vout[pcoin.second].nValue;
                    //The coin age after the next block (depth+1) is used instead of the current,
                    //reflecting an assumption the user would accept a bit more delay for
                ...
                if (nChange > 0 || nBlankChange > 0)
                {
                    ...
                    //       backup is restored, if the backup doesn't have the new private key for the change.
                    //       If we reused the old key, it would be possible to add code to look for and
                    //       rediscover unknown transactions that were written with keys of ours to recover post-backup change.
                    ...
                    if (nChange > 0)
                    {
                        ...
                        if (newTxOut.IsDust(minRelayTxFee))
                        {
                            ...
                            if (!feeFromBlank) nFeeRet += nChange;
                    ...
                    if (nBlankChange > 0)
                    {
                        ...
                        CTxOut newTxOut(nBlankChange, scriptBlankChange);
                ...
                BOOST_FOREACH(const PAIRTYPE(const CWalletTx*, unsigned int)& coin, setBlankCoins)
                    txNew.vin.push_back(CTxIn(coin.first->GetHash(), coin.second, CScript(),
                                              std::numeric_limits<unsigned int>::max() - (fWalletRbf ? 2 : 1)));
